...
Debugging Process
Bug debugger Brennan (BDB... as in my actual initials... made that up on the spot, fair enough right?) encountered the issue upon opening the project and being, well, instructed to address it, roughly 19:20 PST. He promptly proceeded to undergo solving the problem.

Checked running processes using ps aux. Two apache2 processes - root and www-data - were currently running.
...
In one terminal, ran strace on the PID of the root Apache process. In another, curled the server. Expected something wonderful but oh no!, it got disappointed. strace gave no useful information.
...
But of course, it will never occur again, because we're programmers, we fix what is broken everytime! 😉
